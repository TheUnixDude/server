...
function GetPrerequisites()
{
  ...
  if [ $? -ne 0 ]; then
    Log "apt isn't the installer by default" 1
  else
    installer=1
    apt -y install git lsb-release curl
  fi
  ...
  case ${OS} in
    ...
    "Debian")
      case ${VER} in
        "jessie")
          # Debian 8.x
          su -c "apt-get -y install curl build-essential autoconf automake cmake libbz2-dev libace-dev libssl-dev libmysqlclient-dev libtool" root
          ;;
        "stretch")
          # Debian 9.x
          su -c "apt-get -y install curl build-essential autoconf automake cmake libbz2-dev libace-dev libssl-dev libmysqlclient-dev libtool" root
        ...
        "buster")
          # Debian 10.x
          su -c "apt -y install curl build-essential autoconf automake cmake libbz2-dev libace-dev libssl-dev libmariadbclient-dev libtool" root
          ;;
        *)
          OS_VER=0
          ;;
